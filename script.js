anychart.onDocumentReady(function () {
  ...
  for (let i = 0; i < RSIOBnum && i < jsonData.length; i++) {
    ...
    stockLine.textContent = `Ticker: ${jsonData[i].Ticker} | RSI Overbought: ${jsonData[i].RSIOB}`;
  ...
  scanSMA.addEventListener("click", function(){
   ...
   console.log(`you have searched for ${tickSMA}, showing a simple moving average with a time period of ${timeSMA}`);
    ...
    // Iterate over the JSON array and check for ticker matches
  ...
  if (tickermatch = false) {
    ...
    console.log("No ticker match found!");
  }
  })
